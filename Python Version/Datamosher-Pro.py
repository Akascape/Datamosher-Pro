    ...
    if res=="yes":
        try:
            for x in range(len(missingset)):
                y=missingset[x]
                subprocess.Popen('python -m pip install '+y, shell=True)
            messagebox.showinfo("Installing modules...","Modules are being installed...\nPlease restart the program once its done! \n(Make sure you are connected to internet)")
        except:
...
def convert(inputpath,targetformat):
    ...
    try:
        ...
        if highest=="yes":
            ...
            (choice==modes[29])):
                if targetformat==".gif":
                    subprocess.call(f'"{resource}" -i "{inputpath}" "{outputpath}"', shell=True)
                else:   
...
def shuffle(output_video):
    ...
    subprocess.call(f'"{ffgac}" -i "{file}" -an -vcodec mpeg2video -f rawvideo -mpv_flags +nopimb -qscale:v 6 -r 30 -g 90 -s 1280x720 -y "{fin}"', shell=True)
    os.mkdir("cache/raws")
    framelist=[]
    subprocess.call(f'"{ffgac}" -i "{fin}" -vcodec copy cache/raws/frames_%04d.raw', shell=True)
...
def rise(output):
    ...
    if var5.get()==1:
        ...
        qua="1280x720"
    subprocess.call(f'"{ffgac}" -i "{file}" -an -vcodec mpeg2video -f rawvideo -mpv_flags +nopimb -qscale:v 6 -r 30 -g 90 -s "{qua}" -y "{fin}"', shell=True)
    os.mkdir("cache/raws")
    framelist=[]
    subprocess.call(f'"{ffgac}" -i "{fin}" -vcodec copy cache/raws/frames_%04d.raw', shell=True)
    kil=float(killframe.get())
